...
class Settings(surface.Surface):
    ...
    def set_buttons(self):
        ...
        easy_button = surface.ButtonInterface(self.screen, settings_image, 130, 210, "Easy", self.set_level, 1)
        medium_button = surface.ButtonInterface(self.screen, settings_image, 245, 210, "Medium", self.set_level, 2)
        hard_button = surface.ButtonInterface(self.screen, settings_image, 360, 210, "Hard", self.set_level, 3)
        level_buttons_list = [easy_button, medium_button, hard_button]

        # create design buttons
        black_button = surface.ButtonInterface(self.screen, settings_image, 15, 310, "Black", self.set_design, "black")
        dark_blue_button = surface.ButtonInterface(self.screen, settings_image, 130, 310, "Dark Blue", self.set_design,
                                          "dark blue")
        dark_green_button = surface.ButtonInterface(self.screen, settings_image, 245, 310, "Dark Green", self.set_design,
                                           "dark green")
        dark_red_button = surface.ButtonInterface(self.screen, settings_image, 360, 310, "Dark Red", self.set_design,
                                         "dark red")
        dark_gray_button = surface.ButtonInterface(self.screen, settings_image, 475, 310, "Dark Gray", self.set_design,
                                          "dark gray")

        design_buttons_list = [black_button, dark_blue_button, dark_green_button, dark_red_button, dark_gray_button]

        # create menu buttons
        confirm_button = surface.ButtonInterface(self.screen, settings_menu_image, 20, 450, "Confirm",
                                        self.confirm_settings)
        reset_button = surface.ButtonInterface(self.screen, settings_menu_image, 210, 450, "Reset settings",
                                      self.reset_settings)
        exit_button = surface.ButtonInterface(self.screen, settings_menu_image, 400, 450, "Quit", self.exit_settings)
